...
##### <small>3.2</small> 常量的定义 <small>只能是数字、字符串或布尔值</small>
...
* 数组与slice的区别，数组声明时需要长度并大小不可变，slice声明时不需要长度，大小可变，是一个指针，是指向数组的值的指针
* slice的长度: `len(ar)`
* slice的容量: `cap(ar)`
* slice追加: `append(a, 'h')`
...
##### <small>3.8</small> make与new内存分配

`make`是给`map`、`slice` 和`channel`分配内存的
...
##### <small>7.3</small> 缓冲`channel`

```go
ci := make(chan int, x)         // x为缓冲的长度，在x允许的长度内发送与接收都不会阻塞，当超过
```

##### <small>7.4</small> `range`和`close`
