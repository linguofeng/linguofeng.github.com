...
<section>
    ...
    <h3>@.h@ 头文件：<small>由版权版本声明、预处理块、函数和类结构组成。</small></h3>
...
public:
    ...
    <h3>@.cpp@ 定义文件：<small>由版权版本声明、头文件引用、函数实现休</small></h3>
<pre class="prettyprint">
/**
 * 版权版本声明
 * 文件名：test.cpp
...
<section>
    ...
    <h3><small>4.1</small> 运行符优先级</h3>
    <p>运行符太多，在复杂的语句中应使用 @()@ 把要先运行的代码包起来，如： @if ((a == 10) && (b == 10))@ 。</p>

    <h3><small>4.2</small> if语句中的零值比较</h3>
<pre class="prettyprint">
if (flag)
{
    // flag为布尔值的正确写法
...
if (f < 0 || f > 0)
{
    ...
    <h3><small>4.3</small> 循环语句中的效率问题</h3>
<pre class="prettyprint">
for (int i = 0; i < 5; i++)
{
    for (int j = 0; j < 100; j++)
...
// 如果循环体内出现条件判断语句宜将条件判断语句放到循环体外。
    ...
    <h3><small>4.4</small> switch语句</h3>
<pre class="prettyprint">
switch (var)
{
    case 1:
